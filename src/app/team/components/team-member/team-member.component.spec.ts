...
describe('TeamMemberComponent', () => {
  ...
  it('should display the name in h3, the job and desciption in p', () => {
    ...
    // // and a job 
    // const parent = element.querySelectorAll('p');
    // var child_nodes = parent.childNodes;
    // expect(child_nodes.item(0).textContent).withContext('You should have a job for the team member').not.toBeNull();    
    // expect(child_nodes.item(0).textContent).withContext('You should display the job').toBe('job');         
    // // and a description
    // expect(child_nodes.item(1).textContent).withContext('You should have a description for the team member').not.toBeNull();    
    // expect(child_nodes.item(1).textContent).withContext('You should display the foo').toBe('foo');     
  });
